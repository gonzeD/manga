...
def _extract_post_text(item):
    print("extracting")
    actualPosts = item.select(".cxmmr5t8.oygrvhab.hcukyx3x.c1et5uql.ii04i59q")
    print(actualPosts)
    text = ""
    if actualPosts:
...
def _extract_html(bs_data):
    ...
    k = bs_data.find_all(attrs={"role":"article"})
    postBigDict = list()
...
def _login(browser, email, password):
    ...
    browser.find_element_by_css_selector('button[title="Accept All"]').click()
...
def _scroll(browser, infinite_scroll, lenOfPage):
    ...
    while not match:
        ...
        browser.execute_script("document.querySelectorAll('div[role=\"button\"]').forEach(x => x.innerHTML=='See More'?x.click():null)")
...
def extract(page, numOfPost, infinite_scroll=False, scrape_comment=False):
    ...
    browser = webdriver.Chrome(executable_path="/mnt/c/Users/damie/Programmation/winch2.0/chromedriver.exe",options=option)
    print("logging")
